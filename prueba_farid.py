...
def historial():
    ...
    print(f"Total apostado: {contador_total_apostado}")
...
def jugar():
    ...
    global contador_derrotas, contador_victorias, contador_dinero_obtenido,contador_total_apostado
...
# la funcion se repetirá.
    ...
    while dinero > 0:
        print("\n🎲 Opciones de apuesta:")
        print("1️⃣: Número específico (Ganancia: 20:1) Numero (0) (Ganancia: 100:1) ")
        ...
        print("8️⃣: Reiniciar juego")
        print(f"\n 🪙 Tienes {dinero} monedas. 🪙")
        
# Definimos el menú con 7 opciones principales.

        try:
            opcion = int(input("Elige una opción (1-8) o 0 para salir: "))
            if opcion == 0:
            ...
            if opcion == 1:
                ...
                print("¿Esta usteed seguro que desea reiniciar la partida?")
                reiniciar_partida = input("(si/no): ").strip().lower()
                
                while reiniciar_partida not in ['si','no']:
                    reiniciar_partida = input("Ingrese una opcion valida: ")
                    
                if reiniciar_partida == "si":
                    dinero = 100
                    continue
                elif reiniciar_partida == "no":
                    continue
                else:
...
# ademas calculamos las victorias y el dinero ganado
            ...
            if opcion == 1 and apuesta == numero:
                ...
                if dinero == 0:
                    print("💸 Te has quedado sin monedas. ¿Deseas continuar? 💸",flush=True)
                    time.sleep(1)
                    print("1️⃣: Nueva partida")
                    print("2️⃣: Cerrar juego ")
                    nueva_partida = int(input("Elige una opcion: "))
                    
                    while nueva_partida not in [1,2]:
                        
                        nueva_partida = int(input("Opcion no valida. Porfavor, ingrese una opcion valida: "))
                        
                        
                    if nueva_partida == 1:
                        dinero = 100
                        print("Iniciando nueva partida", end="", flush=True)
                        for _ in range(4):
                         time.sleep(1)
                         print(".", end="", flush=True)
                    else:
                        print("Juego terminado, vuelve pronto ;)",flush=True)
                        time.sleep(3)
                        opcion = 0
